...
export default async function AgentsPage() {
  ...
  return (
    <PageShell title="Agent status" description="Per-agent status inferred from tasks, recent file activity, fleet expectations, and available local artifacts. Missing expected agents are shown explicitly so the dashboard matches your intended fleet, not just the currently installed one.">
        ...
        <table className="min-w-full divide-y divide-white/10 text-sm">
          <thead className="bg-white/[0.03] text-left text-zinc-400">
            <tr>
              ...
              <th className="px-4 py-3">Output type focus</th>
              <th className="px-4 py-3">Pending tasks</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {agents.map((agent) => (
              <tr key={agent.id} className={`align-top ${agent.isRegistered ? "" : "bg-white/[0.02]"}`}>
                <td className="px-4 py-4">
                  ...
                  {!agent.isRegistered ? <div className="mt-2 text-xs text-amber-300">Expected fleet member, not currently registered.</div> : null}
                </td>
                ...
                <td className="px-4 py-4 text-zinc-300">{agent.expectedOutputs[0] || agent.latestOutputFile?.name || "No output type profile yet"}</td>
