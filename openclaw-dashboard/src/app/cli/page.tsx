...
export default async function CliPage() {
  const [history, session] = await Promise.all([readCliHistory(30), getCurrentSession()]);
  const canManage = session.profile?.role === "admin" || session.profile?.role === "owner";
  ...
  return (
    <PageShell title="CLI control & debug" description="Allow-listed backend command layer for OpenClaw actions. Unsupported items are shown explicitly instead of being silently ignored.">
      <div className="space-y-6">
        <AdminSyncPanel canManage={canManage} />
