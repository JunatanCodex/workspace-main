...
export function StatusBadge({ value }: { value: AgentHealth | TaskStatus | TriggerType }) {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-medium uppercase tracking-wide ${classes}`}>{key.replace(/_/g, " ")}</span>;
}
